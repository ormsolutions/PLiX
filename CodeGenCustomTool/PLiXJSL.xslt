...
<xsl:stylesheet
	...
	<!--NOTICE!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	Research if this is like using a transaction and if applicable to J#-->
	...
	<xsl:template match="plx:binaryOperator">
		...
		<xsl:if test="$negate">
			<!--NOTICE: Can't negate a whole expression...is that what '('is for??, but can != !!!!!!!!!!!!!!!!!-->
			<xsl:text>!(</xsl:text>
		</xsl:if>
		<xsl:apply-templates select="plx:left/child::*" mode="ResolvePrecedence">
			<xsl:with-param name="Indent" select="$Indent"/>
		...
		<xsl:if test="$negate">
			<xsl:text>)</xsl:text>
			<!--Maybe change whole outer test about negate, can't negate whole expression!!!!!!!!!!!-->
		</xsl:if>
	</xsl:template>
	...
	<!--Condition restrictions for break statements, only in switch case!!!!!!!!!!!!-->
	...
	<xsl:template match="plx:callThis">
		...
		<xsl:choose>
			...
			<xsl:when test="$accessor='explicitThis'">
				<xsl:message terminate="yes">ExplicitThis calls are not supported by C# or J#</xsl:message>
			</xsl:when>
	...
	<xsl:template match="plx:cast">
		...
		<xsl:choose>
			<xsl:when test="$castType='testCast'">
				...
				<!--J# doesn't support the 'as' keyword-->
	...
	<!--Needs to be altered for JEvent and implementation-->
	<xsl:template match="plx:event">
		...
		<xsl:text>Event </xsl:text>
		<!--Class needs to extend System.Object-->
		...
		<xsl:if test="not($isSimpleExplicitImplementation) and not($explicitDelegate) and not(@modifier='override')">
			...
			<xsl:choose>
				...
				<xsl:otherwise>
					...
					<xsl:text>MyEventHandler</xsl:text>
				</xsl:otherwise>
			</xsl:choose>
			<xsl:variable name="typeParams" select="plx:typeParam"/>
			<xsl:if test="$typeParams">
	...
	<xsl:template match="plx:namespaceImport">
		...
		<xsl:value-of select="@name"/>
		<xsl:if test="not(string-length(@alias))">
			<xsl:text>.*</xsl:text>
		</xsl:if>
	</xsl:template>
	<xsl:template match="plx:nullFallbackOperator">
		...
		<!--J# doesn't support this operator-->
		<xsl:text> ?? </xsl:text>
		<xsl:apply-templates select="plx:right/child::*" mode="ResolvePrecedence">
			<xsl:with-param name="Indent" select="$Indent"/>
			<xsl:with-param name="Context" select="."/>
	...
	<xsl:template match="plx:operatorFunction">
		...
		<xsl:variable name="operatorNameFragment">
			<xsl:choose>
				...
				<!--doesn't support cast.explicit or cast.implicit-->
	...
	<!--attributes are still done in /**@attribute*/ way.  Saw one instance tho where it was Attribute att = new Attribute()-->
	...
	<xsl:template name="RenderVisibility">
		...
		<xsl:if test="string-length($Visibility)">
			<!--has transient, static, synchronized-->
